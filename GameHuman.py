...
\tThis will continue until a Player wins."""
	...
	def finish_turn(self, card_indexes, lie):
		...
		"""
		global turn_ct
		print("{0} put down {1} {2}(s).\n".format(
			self.c_player, len(card_indexes), Value.name(self.c_value)))
		...
		if challenges:
			...
			if cheat:
				...
				if dialouges > 0 and dialouges <= 0.2:
					...
					print("{0}: You're too easy to figure out.".format(ch.name))
				else:
					print("{0}: I knew that had to be a lie...got you this time!".format(ch.name))
			...
			# If the player has not cheated:
				...
				if dialouges1 > 0 and dialouges1 <= 0.2:
					...
					print("{0}: I can't believe I fell for your trap".format(ch.name))
				else:
